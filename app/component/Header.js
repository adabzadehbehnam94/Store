...
export default function Header() {
    const { state, user, logoutUser , setSearchAct } = useContext(Redusersdata)
            ...
            <div className={style.bgheader}>
                <div className="container">
                    <div className="row  d-md-flex d-none">
                        ...
                        <div className="col-lg-4  col-sm-6 row  align-items-center justify-content-end text-end">
                            {/* {user ? 
                                <>
                                    <div className="col-3">
                                        <p >{user.name}</p>
                            ...
                            <div className="col-11">
                                <div>
                                    <input onChange={(item) => setSearchAct(item.target.value)} className={`${style.searchHeader}`} type="text" placeholder="Search" />
                                </div>

                            </div>
                            <div className="col-1 position-relative d-flex">
                                ...
                                {state.counteritems > 0 && <span className={`position-absolute ${style.counternumber}`}>{state.counteritems}</span>}
                    ...
                    <div className={` d-md-none row align-items-center justify-content-center position-relative ${style.MobileMenu}`}>
                        <Link className="col-7" style={{ fontSize: "30px" , paddingLeft : "10px" }} href={"/"}>Store</Link>

                        <button className={`col-1 ${style.searchMobileIcon}`} onClick={() => setSearch(Search === "d-none" ? "d-block" : "d-none")} type="button"></button>
                        ...
                        <div className={`${style.searchMobileInput} ${Search}`}>
                            <input onChange={(item) => setSearchAct(item.target.value)} type="text" />
                        </div>
